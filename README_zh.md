...
| [relief-design](skills/relief-design/) | EN/ZH | **用新拟态画技术框图** —— 深度本身就是图例：凸起是一个存在的东西，凹陷是够不到的地方、一个条件、或刻进面料的一道槽。七套皮肤（暖米白 / 中性灰 / 磨砂黑 / 墨黑 / 雾青 / 陶土 / 鼠尾草）由 `data-theme` 切换，两道检查下全部 4.5:1 达标。随附 **8 张范本页** —— [控件](skills/relief-design/references/canonical/controls.html)（四档质感，含一个故意做错的样本，加按钮 / 开关 / 滑块 / 输入框 / 分段）/ [框图](skills/relief-design/references/canonical/diagram.html)（流程链 · 树状 · 左右对比 · 分层调用 · 结构体字段）/ [硬件框图](skills/relief-design/references/canonical/hardware.html)（状态机 · 时序 · 寄存器位域 · IP 内部框图 · 内存布局 · 数据通路 · 时钟树）/ [平台框图](skills/relief-design/references/canonical/platform.html)（启动链 · 分区表 · 编号翻译 · 引脚复用 · 电源域）/ [代码框图](skills/relief-design/references/canonical/code.html)（函数堆栈及三种变体 · 调用关系 · 代码架构 · 运行调度 · 函数时序图 · 结构体指针）/ [排查框图](skills/relief-design/references/canonical/debug.html)（probe 为什么没跑 · 锁持有与死锁环 · 引用计数生命周期 · 竞态窗口 · oops 解读 · 丢失的唤醒）/ [数据结构关系](skills/relief-design/references/canonical/struct.html)（内存布局及填充空洞 · 嵌入/指针/链表三种关系 · 私有指针链 · 设备树属性到字段 —— **每张图左边都贴着它对应的源码**）/ [git 关系图](skills/relief-design/references/canonical/git.html)（分支与合并 · 仓库拓扑 · 补丁流向 · 领先与落后），另有一张 [45 张图集](demos/relief-design/diagrams.html)和一张 [flagship demo](demos/relief-design/index.html) 跟着一次触摸从玻璃走到应用。两道对比度检查缺一不可，因为它们看到的不是一回事：`check_skin_contrast.py` 查**渐变底**上的字（axe 根本算不了渐变背景），`axe-audit.mjs --theme=<皮肤>` 查真实 DOM 每个元素（脚本不扫 DOM）。**不要**用它做 aurora 玻璃（`glass-design`）、应用界面（`atelier-design`）、零基础图解（`primer-design`） |

...
- [**primer 技术图解集**](./demos/primer-design/tech/index.html) —— 七页硬件 / 内核图解，一页答一个问题：寄存器、一次 SPI 传输、一颗 SoC、两块芯片之间的两套门牌号、IP core 内部、一个字从 `write()` 到串口引脚、对半砍
...
| [glass-design](skills/glass-design/) | EN/ZH | 以 **Apple 液态玻璃 / aurora 玻璃拟态** 美学渲染 HTML/CSS —— 深藏青画布 (#0B1020) + aurora 光晕 + 三层毛玻璃面板（真 `backdrop-filter` + 1px 渐变折射环）+ 唯一前景彩色 cyan (#22D3EE)，Space Grotesk + Inter + JetBrains Mono。**dark/light 双主题**（`data-theme`，渲染 / 可达性 / 截图检查双主题各跑一遍）+ **可冻结动画引擎**（`glass.js`：滚动浮现 / count-up / 3D tilt / SVG 路径绘制 / 视差，`prefers-reduced-motion` 下全部塌缩为静态 markup，截图确定性）。为图表 / 图示 / 数据类内容的高冲击力展示而生 —— 见 [图示画廊 canonical](skills/glass-design/references/canonical/diagram-gallery.html) 与 15 件 SVG 图示模板 |
...
| [atelier-design](skills/atelier-design/) | EN/ZH | 渲染**产品应用界面，而不是页面** —— 桃色玫红 mesh 壁纸上浮着**一整块**磨砂应用外壳（#E8AE86 / #E39BA8 停位压在 #E9C3A8 底色上，外壳 0.32 → 侧栏合成 0.46 → 数据卡 0.80 → 表格 0.94，`blur(42px) saturate(1.45)`），渐变圆球图标（珊瑚 #F5854F → 玫红 #DD4F92），灰轨道上的圆头柱状图，每屏恰好一张近黑锚点卡。全局 Plus Jakarta Sans。**JavaScript 是交付物的一半**：侧栏能路由、标签能切换、手风琴能折叠、结果行能展开、表格能排序、开关能翻、KPI 会滚动 —— 全部属性驱动（`data-route` / `data-tab` / `data-sort` / `data-count-to` …），且全部可冻结以保证截图确定性。双主题（亮色为准，暗色是暖咖啡色的 warm-night）。随附 **5 个范本屏** —— [控制台](skills/atelier-design/references/canonical/dashboard.html) / [预订](skills/atelier-design/references/canonical/booking.html) / [记录](skills/atelier-design/references/canonical/detail.html) / [设置](skills/atelier-design/references/canonical/settings.html) / [登录](skills/atelier-design/references/canonical/signin.html)，另有 [9 图图集](demos/atelier-design/diagrams.html)。**不要**用它做落地页（它根本没有 hero 档——应用没有 hero）、深色 aurora 玻璃（`glass-design`）、或任何要打印的东西（`backdrop-filter` 不进打印管线） |
| [primer-design](skills/primer-design/) | EN/ZH | 小白图解读本 —— 厚描边插画 + 比喻卡 + 术语翻译气泡，把任何主题讲到零基础也能看懂 |
| [design-review](skills/design-review/) | EN/ZH | **9 个设计 skill 的独立评审员** —— 四道机械检查（`verify.py` 静态 + `visual-audit.mjs` Playwright 渲染 + `axe-audit.mjs` 可达性（axe-core，color-contrast 阻断；glass light 主题尚有已知欠账，见 known-bugs §6.6）+ `screenshot.mjs` 截图），外加可选 `pixel-gate.mjs` 像素回归（阈值实测校准）与 `critic.mjs` LLM 口味评审 + 仓库内已知 bug 清单。另带 `multi-critic.mjs`（4 个固定权重的专项评审员）、learning loop（critic 抓到的问题经 `design-learner` agent 固化成新的机械检查规则）、`bin/design-review` 命令行（默认跑检查链，另有 `--plan` / `--audit` / `--distill` 模式）。参考 Anthropic 的 [harness design for long-running apps](https://www.anthropic.com/engineering/harness-design-long-running-apps)：generator 和 evaluator 分离，reviewer 不继承 generator 的立场。完整 [9 组件 harness 路线图](docs/HARNESS-ROADMAP.html)（用九种声音里的五种各渲一版） |

...
Ten design skills ship with single-page flagship demos under [`demos/`](./demos/) — same content, nine aesthetics. `hardware-3d` is the odd one out: it renders instead of laying out.
...
- [**anthropic diagram gallery**](./demos/anthropic-design/diagrams.html) — 76 hand-crafted SVG diagrams (registers, SoC blocks, waveforms, schedulers, physical data paths, address spaces, cross-sections, exploded views, pinouts, log-scale rulers, four animated …)
- [**anthropic hardware data path**](./demos/anthropic-design/hardware.html) — three figures on how pixel data physically moves: the display path with no GPU, where the data goes once a GPU is added, and how a discrete card with its own VRAM really does copy across PCIe
- [**panel bring-up**](./demos/anthropic-design/bringup.html) — four figures on what is inside a display module, how its flex is wired, the order its supplies must come up in, and which command to run at each block when it stays dark
- [**how a chip is assembled**](./demos/anthropic-design/packaging.html) — a zoom chain across three orders of magnitude, a process drawn as one part six times, and an exploded view that says what can still come apart
...
- [**primer picture gallery**](./demos/primer-design/diagrams.html) — 27 thick-outline illustrations from the three primers plus the demo (a book's index, one takeout trip, a sealed envelope …), each answering exactly one question
...
| [relief-design](skills/relief-design/) | EN/ZH | **Technical diagrams in neumorphism** — depth IS the legend: raised is a thing that exists, sunken is a place you cannot reach, a condition, or a channel cut into the surface. Seven skins (warm cream / neutral grey / matte black / ink / mist / clay / sage) selected by `data-theme`, all measured clean at 4.5:1 in both gate passes. Ships **8 canonical pages** — [controls](skills/relief-design/references/canonical/controls.html) (four textures including one deliberate near-miss, plus buttons / switch / slider / input / segmented) / [diagram](skills/relief-design/references/canonical/diagram.html) (pipeline · tree · before-after · call layers · struct fields) / [hardware](skills/relief-design/references/canonical/hardware.html) (state machine · timing · register bitfield · IP block diagram · memory map · data path · clock tree) / [platform](skills/relief-design/references/canonical/platform.html) (boot chain · partition layout · interrupt-number translation · pin multiplexing · power domains) / [code](skills/relief-design/references/canonical/code.html) (call stack and three variants · call graph · architecture · execution contexts · sequence diagram · pointer chain) / [debug](skills/relief-design/references/canonical/debug.html) (why probe never ran · who holds what · refcount lifetime · the race window · reading an oops · the missed wakeup) / [struct](skills/relief-design/references/canonical/struct.html) (memory layout with its padding holes · embed vs point vs link · the private-pointer round trip · device-tree property to struct field — **every figure ships the source it explains beside it**) / [git](skills/relief-design/references/canonical/git.html) (branches and merges · clone topology · where a fix travelled · ahead versus behind), plus a [flagship demo](demos/relief-design/index.html) and a [49-figure index](demos/relief-design/diagrams.html) tracing one touch from the glass to the application. Two contrast gates are both required because they see different things: `check_skin_contrast.py` reads text on GRADIENT fills, which axe cannot compute at all, and `axe-audit.mjs --theme=<skin>` reads every DOM element, which the script does not scan. **Don't** use it for aurora glass (`glass-design`), app UI (`atelier-design`), or beginner picture explainers (`primer-design`) |
| [design-review](skills/design-review/) | EN/ZH | **Independent evaluator** for the 10 design skills — gate-chain validation (`verify.py` structural + `visual-audit.mjs` Playwright-rendered + **`axe-audit.mjs` accessibility conformance (axe-core; color-contrast blocking, and as of 2026-08-27 the whole corpus measures clean in both themes — see known-bugs §6.6)** + `screenshot.mjs` + opt-in **`pixel-gate.mjs` visual regression (pixelmatch, thresholds probe-calibrated)** + `critic.mjs` LLM taste review) plus a repo-internal known-bug catalogue. Also ships `multi-critic.mjs` (4 specialist reviewers with fixed weights), a learning loop that turns critic catches into new gate rules via the `design-learner` agent, and the `bin/design-review` CLI (the default gate run, plus `--plan` / `--audit` / `--distill` modes). Inspired by Anthropic's [harness design for long-running apps](https://www.anthropic.com/engineering/harness-design-long-running-apps): generator and evaluator are separate skills so the reviewer doesn't inherit the generator's assumptions. See the full [9-component harness roadmap](docs/HARNESS-ROADMAP.html) (rendered in five of the ten voices) |
...
| [design-planner](skills/design-planner/) | ZH | **Brief→sprint-contract planner** for the 10 design skills — expands a vague one-line brief into page-type + audience + section plan + hard quotas (diagram density / bilingual / brand) before any HTML is written, wrapping `bin/design-review --plan`. Unknown page-types borrow the nearest canonical structure and are stamped LOW-CONFIDENCE |
...
- **Diagrams** — 49 SVG templates with orange/blue/green category coloring and diamond decision gates, including kernel-grade types (register-bitfield / soc-block / hw-timing-waveform / sched-timeline / protocol-stack / address-map / soc-dataflow / gpu-dataflow / vram-dataflow / platform-block / address-spaces / copy-vs-share / dataflow-animated / page-flip / gpu-memory-flow / vram-copy-flow / arm64-display-path / display-controller-inside / dsi-panel-inside / bus-tenants) plus **two** component sheets (`glyphs.svg`, `glyphs-memory.svg`) — browse them all in the [60-figure gallery](demos/anthropic-design/diagrams.html)

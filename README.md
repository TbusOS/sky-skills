...
- [**primer technical set**](./demos/primer-design/tech/index.html) — seven hardware / kernel explainers, one question each: a register, one SPI transfer, an SoC, two chips whose house numbers never merge, an IP core's insides, a character's path from `write()` to a UART pin, and halving
...
- **Diagrams** — 15 SVG templates with orange/blue/green category coloring and diamond decision gates, including kernel-grade types (register-bitfield / soc-block / hw-timing-waveform / sched-timeline / state-machine / deployment) — browse them all in the [24-diagram gallery](demos/anthropic-design/diagrams.html)

...
> **更正(2026-08-25):§4.1 / §4.2 整段分析用错了匹配器,下面的推理已撤回,别照它做。**
> 串味检查读的是 computed color、按**欧氏距离 < 22** 判命中;"逐通道 ≤ 55"是另一个匹配器
> (brand-presence,截图像素上的欧氏 55)的容差。两个匹配器的准确说明见
> `skills/design-review/scripts/visual-audit.mjs:105-114`,实测数字见同文件 313-325 行。
> 结论侧的影响:§4.1 的三个"必须豁免"其实都不必要(实测 43.3–94.3,阈值 22);
> §4.2 的"预计豁免 glass / lectern"也不成立 —— 九家最后全部加了 primer 紫;
> §4.3 的黄单档铁律**规则本身留着,但它的理由换了** —— 压深的黄距 ember 金实测 60.5,
> 阈值 22,而且 ember 金不在 primer 的禁忌清单里,所以没有任何机器检查会为它响。
> 它现在立在读感上:压深的黄不再像荧光笔划痕,开始像 ember 的暖金。
> 本节保留原文作为决策记录,**它不是现行指引**。

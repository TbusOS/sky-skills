...
2. **黄只有一档。** 见 `dos-and-donts.md` §2 的铁律和它的理由(口味规则,没有机器兜底)。
3. **绿色区域里的文字仍是 `--primer-ink`。** 绿在纸上只有 2.94:1,它是勾和轨道的颜色,不是字的颜色。

### 三家没有写进 primer `forbiddenColors` 的招牌色

串味检查读的是可见元素的 computed `color / background-color / fill / border-color`,
按**欧氏距离 < 22** 判命中(`visual-audit.mjs:105-114`)。渐变、阴影、描边它读不到,
抗锯齿混出来的像素它更读不到 —— 那是另一个匹配器(brand-presence,截图像素上的欧氏 55)的事。

这里原来写的是"逐通道 |Δ| ≤ 55,三家必然重叠,所以必须豁免",那是把两个匹配器当成了一个,
**已撤回**。按真匹配器,2026-08-25 扫过每一张引用 `primer.css` 的页面,最近的一对是:

| 没列进去的色 | primer 侧最近的实色 | 欧氏距离 | 阈值 |
|---|---|---|---|
| ember 金 `#c49464` | 马克笔黄 `#ffd23f` | 93.2 | 22 |
| sage 绿 `#97B077` | "懂了"绿 `#3aa66b` | 94.3 | 22 |
| lectern `#1d3a6e` | 正文墨 `#243244` | 43.3 | 22 |
| lectern `#2f5bb0` | 深紫强调 `#5b3fbf` | 54.3 | 22 |

四项全部远在 22 之外 —— 三家其实都**可以安全列进禁忌表**,不列不是因为会误报。
往清单里加色是行为改动,不在写这几行字的任务范围里,留作单独一项跟进。
在它落地之前:真去用 ember 金或 lectern 蓝当强调色,机器抓不到,critic 会抓。

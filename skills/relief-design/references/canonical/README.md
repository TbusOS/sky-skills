...
八张参考页。每张都是**这个风格在一种页型上的完整样子**，不是片段集。

| 文件 | page-type | 内容 |
|---|---|---|
| `controls.html` | `controls` | 四档质感对照（含一个故意做错的样本）+ 按钮 / 开关 / 滑块 / 输入框 / 分段控件 |
| `diagram.html` | `diagram` | 流程链 · 树状 · 左右对比 · 分层调用 · 结构体字段 |
| `hardware.html` | `hardware` | 状态机 · 时序 · 寄存器位域 · IP 内部框图 · 内存布局 · 数据通路 · 时钟树 · 缓存一致性 · DMA 描述符链 · 总线事务时序 + 术语表 |
| `platform.html` | `platform` | 启动链 · 分区表 · 编号翻译（中断号）· 引脚复用 · 电源域 |
| `code.html` | `code` | 函数堆栈 · 调用关系 · 代码架构 · 运行调度 · 函数时序图 · 两栈对比 · 栈溢出 · 中断栈 · 结构体指针 · **缩进式调用树（ftrace 那种样子）· 指针表的调用去向** |
| `struct.html` | `struct` | 结构体内存布局 · 嵌入/指针/链表三种关系 · 私有指针链 · 设备树属性到字段 · **位域与 `__packed` · 联合体**（**每张图左边贴着它的源码**）|
| `debug.html` | `debug` | probe 为什么没跑 · 锁持有与死锁环 · 引用计数生命周期 · 竞态窗口 · oops 解读 · 丢失的唤醒 · 越界落点 · dmesg 空档 |
| `git.html` | `git` | 分支与合并 · 仓库拓扑 · 补丁流向 · 领先与落后 |

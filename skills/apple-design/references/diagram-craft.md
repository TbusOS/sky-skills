...
| call-site-locator | **那道耦合**：两枚同名标签 + 它们之间那条线 + 代码里那个共享文件名 | 层级卡 / `file:line` 列 / 深色卡全灰 |
...
### 17.5 调用链定位图（call-site-locator）的 apple 译法

图型工艺在 anthropic-design `diagram-craft.md §15.50`；这里只写 apple 不一样的几处。
**这张图不手画**：`skills/design-review/scripts/gen_call_site_figure.py --style=apple`，
源格式见 `skills/design-review/references/callsites/README.md`。下面这几条就是生成器的排版规则。

**一、两条链并排，不上下叠。** anthropic 版是上下两组，apple 版是左右两组 ——
不是排版偏好，是 apple 的留白配额逼出来的，而逼出来的结果更对：
上下叠会被读成「先这个，再那个」，而这张图的全部要点恰恰是**这两条链彼此不调用**。
并排天生说得清这件事，省掉一句解释。
生成器先量两条链各自要多宽，**放得下才并排，放不下就上下叠 —— 宁可叠也不把字缩小**。
为了放得下，长的副说明和「给不出行号」的理由折成两行：生成器第一版就是被一句
50 个字符的副说明撑出 385px 的卡，两条链并排差 87px，整张图退回上下叠、高到 1112。

**二、蓝给耦合，不给焦点卡。** §2 的规格里焦点卡是「白底 + 蓝 1.5px 描边」，
这张图**不用**那一条 —— 蓝已经许给了两枚标签、它们之间那条线、以及代码里那个
共享的文件名，三件同属一个叙事（这个文件是两条链唯一的交点）。
再给焦点卡一道蓝边，读者就分不出「这一层最重要」和「这件事是结论」了。

**三、「你要找的那一行」靠明度，不靠色相。** 焦点卡让出了蓝，那一行由深色卡里的
明度台阶接手：普通代码 `#86868b`、**决定结局的那几个字 `#ffffff`**。
这正是 §16.2「去掉色相之后类别只剩明度可用」在一张图里的具体用法。
共享文件名仍然用 `#0071e3` 0.26 的色块 —— 它属于耦合那个叙事，不属于这一个。

**四、`file:line` 列左对齐，不右对齐。** §17.4 记过右对齐行标签把左边界顶穿的坑；
这里同样的问题会出在最长的那个出处上（`check_coverage.sh:98-103`）。
左对齐钉一个固定 x，各行出处的**左**边缘成列，长短不一往右伸，
伸不出去就写短一点 —— 出处是给人抄去 grep 的，缩写不得。

**为什么不画引导点线**：anthropic 版用目录页那种点线把卡片和出处连起来。
apple 少用虚线（§4），而且这张图两组各自只有 520 宽，卡片到出处的距离本来就短，
留白足够让眼睛自己走完。**点线在这里是多出来的一道笔画**，
删掉之后图更安静，信息一点没少。

**五、被删掉的那次调用画成虚线幽灵卡**（没有底色，透出分组的 `#f5f5f7`），名字用 `#86868b` 划掉；
它下面的层是白卡、去掉投影、名字变灰 —— 代码在，只是不会跑了。**两种都不用蓝**，
蓝已经许给了耦合。apple 的虚线只留给「未定 / 回流」（§4），「这次栈上没有它」正属于前一种。

**六、失败往上传到哪**：焦点那棵子树之后多一行，`#aeaeb2` 灰虚线从出事那层的缩进往左伸回落点，
返回值用 `#1d1d1f` 等宽粗体、说明用 `#86868b`。回流用虚线，和 §4 的规矩一致。

**七、手机上不缩小，横向滚动**：图集页里套一层 `.gal-scroll`（`min-width: 840px`，
按最小字 12.5px 渲染后不低于 9px 算出来的）。不套的话整张图等比缩到 284px 宽，
最小的字实测只剩 **3.2px**。

...
| `.anth-hero` 内层窄块只写 `max-width` 不写 `margin:auto` | text-align:center 只居中**文字**,块本身 margin-left 算成 0、左贴边,整块版心偏左 ~166–310px,肉眼以为居中。hero 内层块直接用 `.anth-container`(自带 `margin:0 auto`),或显式补 `margin-left:auto; margin-right:auto`。max-width 和 margin:auto 成对出现,缺一不可。anthropic.css 已加 `.anth-hero .anth-container > * { margin-inline:auto }` 兜底,但**只救得了外层块**:内层 `<p>` 若写行内 `margin: … 0`,行内样式压过样式表,仍会在块内偏左(实测 50px)。副标一律写 `margin: var(--space-5) auto`。**known-bugs 1.46** |
| 行内 `<code>` 套 `.anth-code` | `.anth-code` 是块级 `<pre>` 的样式(padding:32px),套到行内代码上把盒撑到 ~80px 高、压叠上下行,引发一批 text-overlap。行内代码用裸 `<code>`(anthropic.css 已给 padding:2px 6px),`.anth-code` 只留给块级 `<pre>`。anthropic.css 已加 `code.anth-code:not(pre code)` 回落成行内度量,误套不再撑高;但自带内联样式、不加载 anthropic.css 的导出页没有这层保护。**known-bugs 1.47** |
| 整张卡片用 `<a>` 包裹却不给卡内 `<h3>`/`<p>` 复位 color | 标题/正文继承 anchor 的链接色(橙/蓝/紫),不是正文墨色,整块读成"巨型链接"。anthropic.css 已加 `a.anth-card{display:block;color:inherit;text-decoration:none}` 兜底,整卡链接不再吃浏览器默认链接蓝;自定义卡片类(如 `.navcard`)仍要自己复位:`.navcard h3,.navcard p{color:var(--anth-text)}`(正文用 `--anth-text-secondary`)。或别整块套 `<a>`,只在标题 / "了解更多" 处放链接。**known-bugs 1.48** |

...
| Actor 3(数据/资源) | `#a85730` 红 | storage / DB / cache |
...
```xml
<!-- ❌ 错 · 浏览器整 <svg> 解析中断 · 全空白 -->
<text>每次请求验 <strong>JWT</strong> 签名</text>
<text>读 <code>session_store</code></text>

<!-- ✅ 对 · 全 plain text · 用 font-family attribute 切换字体 -->
<text font-family="Poppins, sans-serif">每次请求验 JWT 签名</text>
<text font-family="JetBrains Mono, monospace">读 session_store</text>
```
...
- **登录鉴权 4 步流程**(3 actor:Client / Auth API / Session store)—— client 发登录请求 → Auth API 校验凭证并签发 token → 写 session store → 返回 token。经典的 "client → server → 存储" 三段式时序。

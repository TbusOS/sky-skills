#!/usr/bin/env python3
"""
Verify a rendered HTML against anthropic-design invariants.

Usage:
  python3 skills/anthropic-design/scripts/verify.py path/to/output.html [...]
...
CSS_PATH = os.path.join(SKILL_DIR, "assets", "anthropic.css")
...
NARROW_HERO_CONTAINERS = {"anth-container--narrow"}
ACCEPTABLE_HERO_CONTAINERS = {"anth-container", "anth-container--wide"}


class HeroContainerFinder(HTMLParser):
    """Finds the first element inside <section class="anth-hero">."""
...
def defined_classes(css_text: str) -> set[str]:
    return {m.group(1) for m in re.finditer(r"\.(anth-[a-z0-9-]+)", css_text)}
...
def check_file(path: str) -> list[str]:
    ...
    if finder.found is not None:
        ...
        if tokens.intersection(NARROW_HERO_CONTAINERS) and not tokens.intersection(
            ...
            if not any(t.startswith("anth-container") for t in tokens):
                errors.append(
                    f"{path}: hero inner element lacks any .anth-container* class ({finder.found!r})"
                )

    # 4. class usage — all classes used must exist in anthropic.css
    if not os.path.exists(CSS_PATH):
        errors.append(f"anthropic.css not found at {CSS_PATH}")
    else:
        defined = defined_classes(open(CSS_PATH, encoding="utf-8").read())
        for cls in sorted(used_classes(html)):
            if cls not in defined:
                errors.append(f"{path}: undefined class '{cls}' (not in anthropic.css)")
    ...
    #    without the base .anth-container will lose margin: 0 auto centering.
    for m in re.finditer(r'class="([^"]+)"', html):
        classes = m.group(1).split()
        for cls in classes:
            if re.fullmatch(r"anth-container--(?:narrow|wide)", cls):
                if "anth-container" not in classes:
                    errors.append(
                        f"{path}: '{cls}' used without base 'anth-container' — "
                        f"margin:0 auto won't apply; write class=\"anth-container {cls}\""
                    )
...
def main() -> int:
    ...
    if failures:
        print("anthropic-design verify: FAIL")
        for line in failures:
            print(f"  • {line}")
        print(f"\n{len(failures)} issue(s). See references/dos-and-donts.md.")
        return 1
    print(f"anthropic-design verify: OK — {len(sys.argv) - 1} file(s) passed")
    return 0

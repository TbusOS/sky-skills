#!/usr/bin/env python3
"""
Verify a rendered HTML against sage-design invariants.

Usage:
  python3 skills/sage-design/scripts/verify.py path/to/output.html [...]
...
CSS_PATH = os.path.join(SKILL_DIR, "assets", "sage.css")
...
NARROW_HERO_CONTAINERS = {"sage-container--narrow"}
ACCEPTABLE_HERO_CONTAINERS = {"sage-container", "sage-container--wide"}


class HeroContainerFinder(HTMLParser):
    """Finds the first element inside <section class="sage-hero">."""
...
def defined_classes(css_text: str) -> set[str]:
    return {m.group(1) for m in re.finditer(r"\.(sage-[a-z0-9-]+)", css_text)}
...
def check_file(path: str) -> list[str]:
    ...
    if finder.found is not None:
        ...
        if tokens.intersection(NARROW_HERO_CONTAINERS) and not tokens.intersection(
            ...
            errors.append(
                ...
                f"expected sage-container (default 960px) or sage-container--wide (1200px)"
            )
        elif not tokens.intersection(ACCEPTABLE_HERO_CONTAINERS):
            # hero exists but container is non-apple — warn if it's non-standard
            if not any(t.startswith("sage-container") for t in tokens):
                errors.append(
                    f"{path}: hero inner element lacks any .sage-container* class ({finder.found!r})"
                )

    # 4. class usage — all classes used must exist in sage.css
    if not os.path.exists(CSS_PATH):
        errors.append(f"sage.css not found at {CSS_PATH}")
    else:
        defined = defined_classes(open(CSS_PATH, encoding="utf-8").read())
        for cls in sorted(used_classes(html)):
            if cls not in defined:
                errors.append(f"{path}: undefined class '{cls}' (not in sage.css)")
    ...
    #    without the base .sage-container will lose margin: 0 auto centering.
    for m in re.finditer(r'class="([^"]+)"', html):
        classes = m.group(1).split()
        for cls in classes:
            if re.fullmatch(r"sage-container--(?:narrow|wide)", cls):
                if "sage-container" not in classes:
                    errors.append(
                        f"{path}: '{cls}' used without base 'sage-container' — "
                        f"margin:0 auto won't apply; write class=\"sage-container {cls}\""
                    )
...
def main() -> int:
    ...
    if failures:
        print("sage-design verify: FAIL")
        for line in failures:
            print(f"  • {line}")
        print(f"\n{len(failures)} issue(s). See references/dos-and-donts.md.")
        return 1
    print(f"sage-design verify: OK — {len(sys.argv) - 1} file(s) passed")
    return 0

#!/usr/bin/env python3
"""
Verify a rendered HTML against ember-design invariants.

Usage:
  python3 skills/ember-design/scripts/verify.py path/to/output.html [...]
...
CSS_PATH = os.path.join(SKILL_DIR, "assets", "ember.css")
...
NARROW_HERO_CONTAINERS = {"ember-container--narrow"}
ACCEPTABLE_HERO_CONTAINERS = {"ember-container", "ember-container--wide"}


class HeroContainerFinder(HTMLParser):
    """Finds the first element inside <section class="ember-hero">."""
...
def defined_classes(css_text: str) -> set[str]:
    return {m.group(1) for m in re.finditer(r"\.(ember-[a-z0-9-]+)", css_text)}
...
def check_file(path: str) -> list[str]:
    ...
    if finder.found is not None:
        ...
        if tokens.intersection(NARROW_HERO_CONTAINERS) and not tokens.intersection(
            ...
            errors.append(
                ...
                f"expected ember-container (default 960px) or ember-container--wide (1200px)"
            )
        elif not tokens.intersection(ACCEPTABLE_HERO_CONTAINERS):
            # hero exists but container is non-apple — warn if it's non-standard
            if not any(t.startswith("ember-container") for t in tokens):
                errors.append(
                    f"{path}: hero inner element lacks any .ember-container* class ({finder.found!r})"
                )

    # 4. class usage — all classes used must exist in ember.css
    if not os.path.exists(CSS_PATH):
        errors.append(f"ember.css not found at {CSS_PATH}")
    else:
        defined = defined_classes(open(CSS_PATH, encoding="utf-8").read())
        for cls in sorted(used_classes(html)):
            if cls not in defined:
                errors.append(f"{path}: undefined class '{cls}' (not in ember.css)")
    ...
    # 6. Modifier-only container (BEM bug): '--narrow' / '--wide'
    #    without the base .ember-container will lose margin: 0 auto centering.
    for m in re.finditer(r'class="([^"]+)"', html):
        classes = m.group(1).split()
        for cls in classes:
            if re.fullmatch(r"ember-container--(?:narrow|wide)", cls):
                if "ember-container" not in classes:
                    errors.append(
                        f"{path}: '{cls}' used without base 'ember-container' — "
                        f"margin:0 auto won't apply; write class=\"ember-container {cls}\""
                    )
...
def main() -> int:
    ...
    if failures:
        print("ember-design verify: FAIL")
        for line in failures:
            print(f"  • {line}")
        print(f"\n{len(failures)} issue(s). See references/dos-and-donts.md.")
        return 1
    print(f"ember-design verify: OK — {len(sys.argv) - 1} file(s) passed")
    return 0

...
- **同类要一起查的**：固定定位的控件（语言按钮、皮肤条、导航）在窄屏下会叠在一起。
  手机宽度下的**重叠**同日又量出 29 处、已修（四家导航换行后栏高写死、hardware-3d 叠层界面、
  atelier booking 五列表单、apple 模板吸底条），窄屏扫描在 390 / 768 两个宽度下也跑重叠
  （warn）。**还开着**：82 页 356 张 SVG 在 390px 下缩到字号 < 8px（8743 处文字），
  是「图在手机上缩到读不了」一类，修法（拖 / 重画窄版）未定。
...
- **同日又两种**：① **祖先透明度为 0**：hardware-3d 被说明面板遮住的场景标注，外层 `.lb` 淡到 0，
  里面的字自己还是 1，检查只读字自己的透明度 → 改用 `checkVisibility({checkOpacity})`（它看祖先）。
  ② **贴底浮层**：吸底购买条遮住滚到它下面的正文，这是它的用途，遮住哪段只取决于滚到哪 →
  一边是贴底浮层（sticky/fixed、bottom 有值、top 为 auto）、另一边是正常排版时不算；
  两个浮层互相压照报（hardware-3d 的语言按钮压在面板的 Next 上就是这种）。各配一对探针。
...
### 1.70 居中的内容比横向滚动容器宽 —— 左边那段永远拖不到

- **Reader sees**：relief diagram 在 390px 下，流水线图的前 308px 看不见，树形图最左边的节点
  被切掉一半，而且连线和节点对不上。怎么左右拖都拖不出来。
- **Why**：滚动容器只能从左边界往右滚。`justify-content: center`（或 `place-items: center`）
  的一行比容器宽时往**两边**溢出，左边溢出的那段落在滚动原点之前，没有任何办法拖到。
  这是 §1.66 的修法（让图板在窄屏下自己横向拖）带出来的：以前整页横滚，至少能看到。
- **为什么没被抓**：它不会让整页变宽，横滚检查看不见；没有重叠；每道检查都绿。是截图看出来的。
- **Defense**：visual-audit 新检查 `pan-unreachable`（error）：滚动位置为 0 时，横向滚动容器里
  有可见内容伸到左边界外即报 —— 按裁剪框算（装饰圆超出自己的 `<svg>` 被裁掉，不算）。
  窄屏扫描在 390 / 768 两个宽度下每次都跑它。修法：`safe center`（放得下居中、放不下从左排起），
  或给内容一个最小宽度让它从左边排起。反向验证：改动前的 relief.css 上报出这两张图（307px / 97px）。

...
8. **单色图反向闸（diagram-monochrome · known-bugs §1.30 · 仅 anthropic）**：figure 内工程图（节点 rect ≥ 4 且 text ≥ 6）的 fill/stroke 中 0 个饱和 hue（s > 0.25 且 0.15 < l < 0.85）→ warn。颜色必须在场：tint 不算 hue，在场感来自实心色点 / 徽章 / 色条 / 彩色连线。"每图 ≥ 2 hue"是文档合约 + critic 评审项（机器只抓 0 hue，存量单 hue git graph / timeline 合法）。apple 豁免（灰阶 + 蓝单焦点是身份）。

...
九套语言是通用的,一个用它们的项目却有自己的决定:写哪一套、哪些提示看过了并且
决定就这样、这一页到底想让谁看懂什么。这些今天没有落点,于是**每次跑都在重新争
同样的 warning**,而口味评审判的是一个虚构记事本应用的样张,不是这个项目的意图。
...
1. **理由必填且要写成句**。这是豁免可复核与静音开关的分界。
2. **`check` 必须是真实存在的检查 id**。id 是从 `visual-audit.mjs` 源码里现读的,
   所以上游改名会在这里报错,而不是让豁免安静地什么都不做。
...
- `pixel-gate.mjs`(`--pixel`)— 可选的第六道机械检查,像素回归比对已提交基线。
  **每张基线旁边有一份同名 `.json`,记着它是在什么环境下录的**(Chromium 版本、
  视口、真正加载成功的字体)。比对前先比这些,不一样就单独报「环境变了,不是页面变了」,
  退出码 **3**,跟真回归的 1 分开。

  为什么要分开:2026-09-11 那 7 张基线全部报回归(dashboard 1.65%),查了一下午才确定
  **页面一个字没改** —— 基线录于 Chromium 147,机器换到了 148。把图缩到 1/16 差异归零,
  说明差的是字形边缘而不是有东西移位;换回 147 渲染,差距缩小 4.5 倍。

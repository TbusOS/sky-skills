...
> 每条带版本区间，且必须**按目标树用 fact-gate 核实**——不靠记忆。
> 候选条目由 `version_drift.mjs`（P4 建）对多版本树跑出来后，经回归门入库。
...
## 种子候选（⚠ 待 fact-gate 按目标树核，未经核实勿当结论）

> 下列是常被提到的版本敏感点，**P1 fact-gate 建好后逐条对真树验证**再转正式条目。现仅作待核清单。

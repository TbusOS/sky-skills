...
把 `linux-kernel-dev` 从一份静态文档，改造成一个**越用越聪明**的 kernel/BSP 专家：每次真任务沉成一条 eval case，每个踩过的坑沉成一条**带回归证明**的规则，技能内容只在「通过冻结语料且不退步」时才升级。并且**跟着内核版本自更新**——版本敏感的知识带版本标、按目标版本树自动验证、版本变化由 drift 检测器机器报出来，不靠手维护（见 §6.9）。机制跟 sky-skills 已有的 design harness 同构，能复用的现成代码就复用。
...
| 词 | 大白话 | 内核里的例子 |
|---|---|---|
| **闸 (gate)** | 必须通过的检查，过不了就拦下 | `checkpatch.pl` 报 error 就不让过——它就是一道闸。本套三道闸：checkpatch(风格) / fact-gate(引的 API/CONFIG 在真树查得到吗) / 编译(模块能 make 过吗) |
| **账本 (ledger)** | 一个流水记录文件(表格 .tsv) | 每次改完测完记一行："改了啥 / 分数 86→89 / 留下还是撤销" |
| **回归门** | 改一次跑一次回归、不退步才保留 | 改动只有"修好≥1 个错且没搞坏别的"才留下，否则 `git revert`。质量只涨不跌 |
| **critic (打分员)** | 当评审的 LLM，给答案按角度打分 | 4 角度：对不对 / 风格 / 安全 / 完整 |
| **eval 语料 / 题库** | 固定测试题 + 标准答案 | 量"skill 改完变好还是变坏" |
| **红线用例** | 碰了就单独报警的测试题 | "绝不能碰禁止路径"，单独算分，不被平均盖过 |
| **fires / catches** | 规则被触发几次 / 真逮到错几次 | 老触发从没逮到错的规则 = 没用，该删 |
| **baseline (基线)** | 改动前的成绩快照 | 改完跟它比，看有没有退步 |
| **冻结 (frozen)** | 固定不变的那份 | 题库和闸在改 skill 内容时不动，否则没法公平比较 |
| **`[CLAIMS]` 块** | 答案末尾列出可被机器核对的断言 | 列出引的 API / CONFIG / 符号，给 fact-gate 当核对靶子 |

> **一句话串起来：** 每改一次 kernel skill 内容，就拿固定题库跑三道闸 + 打分员；改动只有"修好了错又没搞坏别的"才留下（回归门），全程记进账本——这样它只会越来越准。
...
| **routing_accuracy eval 语料** | 变成 kernel 任务 eval 语料 + ground truth | **强用** |
| **references/ 渐进披露** | 把 643 行拆成按需加载的深潜文档 | **用** |
| **拿真源码树做事实校验** | fact-gate vs 真 kernel 树（树无关 `--tree`） | **强用** |
| L1/L2/L3 路由、cross_skill_triggers、MMU 框架 | 单技能不需要路由编排 | 不用 |

一句话：Android-Software 证明了"kernel 知识技能不该是静态文档"——该有 forbidden actions + 可度量记忆 + eval + scripts + 版本跟踪。

### 3.2 ai-doc 论文（借自我进化循环）

精读 18 篇后的可用思维（完整综合见 **附录 A**）。**地基规则（4 篇独立收敛）**：

> **没有可执行检查，就不准建笔记。** 每条 gotcha 必须原子地带上 ① 可 grep 的 `[CLAIMS]` ② 一条"建前 fail / 建后 pass"的冻结 eval case ③ rules 注册。缺一个就拒绝。

这条直接修掉"笔记写完没人验证它真防住复发"的开环问题。其余高价值条目：

| # | 思维 | 落点 | 工作量 |
|---|---|---|---|
| 1 | 原子三件套（地基规则） | auto-learn | M |
| 2 | fires/catches + Laplace `s=(c_succ+1)/(c_use+2)` + leave-one-out 消融 prune；git-revert 是默认 | ledger | M（design-evolve 已有，搬） |
| 3 | 能力红线用例 case 单独门 + objective/subjective 标签，keep/revert 只看 objective 半 | eval-corpus | **S** |
| 4 | gotcha 蒸成抽象通用原则（具体路径只留 must-cite）+ 建前去重 + 邻域 case | auto-learn | M |
| 5 | 答题走有界多轮自纠，gate 失败原样喂回；只在 fail→pass 翻转时落笔记 | fact-gate | M（推后） |
| 6 | router 按召回评分 + 不确定门 | router | 单技能基本不适用 |
| 7 | 生成式 self_model.json + strategies.json 学习策略菜单 | other | L（推后） |
| 8 | 写侧 index.md + 追加式 log.md + lint + 语料通过率主指标曲线 | memory | M |
...
| 源（路径不入库） | 是什么 | 在引擎里当什么 |
|---|---|---|
| **源码树**（`--tree`） | 一棵 6.x 内核源码树 + bootloader（自带 checkpatch.pl） | **fact-gate 源码靶子**：查 API/符号/CONFIG/compatible **实存**；跑 checkpatch；（可选）真编译 out-of-tree 模块 |
| **文档树**（`--docs`） | mainline 内核文档树（如 7.0，`Documentation/*.rst`） | **内容权威源**：写 `references/` 时对齐 `coding-style.rst` / `submitting-patches.rst` / `devicetree/bindings/writing-{bindings,schema}.rst` / `kbuild/{kconfig-language,makefiles}.rst` / `locking/` / `memory-barriers.txt` / `atomic_t.txt` 等；**约定类 fact-check 的靶子**；**eval 种子源** |
...
| canonical 页面语料 | eval 语料：kernel/BSP 任务 case | 新建 |
| `verify.py`（结构闸） | **`checkpatch.pl` 包装**（kernel 树自带，可 `--no-tree` 独立跑） | 新建薄包装 |
| `visual-audit.mjs`（启发式渲染闸） | **fact-gate**：引的 API/CONFIG/符号在真树存在 + 可选**编译闸** | 新建 |
| `multi-critic.mjs`（4 加权 LLM） | kernel critic 面板：correctness / coding-style / safety / completeness | 照 `multi-critic.mjs` 改 |
...
| `design-learner`（learning-loop） | `/kernel-learn`：catch → 规则 + eval case + 注册 | 照 agent 改 |
| `design-evolve`（回归门） | kernel 回归门：keep/revert 内容改动 vs 冻结语料 | 复用机制 |
...
**kernel 比 design 的优势**：checkpatch.pl / 编译 / API 实存都是现成确定性硬闸，比 design 的 visual-audit 启发式更硬。
...
真做一个 kernel/BSP 任务
  ...
  → checkpatch 闸（coding-style，确定性）
  → fact-gate（CLAIMS vs 真 kernel 树 grep）→ 客观 pass/fail（0 幻觉 = 过）
  → critic 面板（correctness / coding-style / safety / completeness）→ 加权分
  → 踩坑 / critic 抓到问题
  → /kernel-learn：原子落 ① known-bug/dos-donts 规则（带 [CLAIMS]）② fail→pass eval case ③ rules 注册
  → 回归 runner 跑全语料：新内容不让旧 case 退步 + 修好新 case → keep 进 ledger；否则 git-revert
  → 下次同类坑：eval case 证明不复现；规则 catch +1；高 fires 零 catch → 消融 prune
```

度量"是否真的越用越聪明" = **语料通过率随 commit 的曲线**（主指标，论文 #8）。
...
没有 `[CLAIMS]` 块 = 红线用例 fail。这是 fact-gate 和 critic 的靶子。

### 6.2 fact-gate（`scripts/fact_gate.mjs`，树无关）
...
### 6.3 checkpatch / compile 闸（`scripts/checkpatch_gate.sh` / `compile_gate.sh`）
...
### 6.4 eval 语料（`tests/eval/cases/*.json`，冻结）
...
- 只收**有区分度**的 case（所有版本都轻松过的丢掉），保持语料小而锐。

### 6.5 critic 面板（`scripts/kernel-critic.mjs` + critic agent 类型）
...
### 6.6 ledger 回归门（复用 design-evolve 机制）

- 每次内容改动 = 一个 experiment：lock baseline → 改 → 跑语料 → **keep iff 修好 ≥1 失败 case 且 0 退步，否则 git-revert（默认结局）**
- 每条规则带 `c_use` / `c_succ` / `last_fired`，`s=(c_succ+1)/(c_use+2)`（Laplace，design-evolve 已实现）
- 周期 **leave-one-out 消融**：逐条停用规则重跑语料，零边际贡献（高 fires 零 catch）→ prune（archive 不删）
...
2. 一条 eval case，跑 fact-gate **建前 fail / 建后 pass**，不翻转就**拒绝建笔记**
...
| | 版本稳定核（占大头，不打版本标，永远教） | 版本易变面（打版本标 + 按目标版本过 fact-gate） |
|---|---|---|
...
- **fact-gate 多版本**：fact-gate 本就树无关（`--tree`）。同一套题库**跑多棵版本树**：一条 claim 在 6.1 过、6.12 fail → 它版本敏感、自动该打标；eval case 带 `valid_versions`，gate 挑匹配的树跑。
- **drift 检测器** `scripts/version_drift.mjs --tree <新版本树>`（把 dirty_pages 变成机器可执行）：重跑版本敏感 claim/case，报告哪些 失效（API 移了/改了/没了）→ 喂给 `/kernel-learn` 生成版本条件更新，过回归门。
- **自更新循环（版本维度）**：
  ```
  出新内核版本 / 换目标 BSP 版本
    → 拿它的树跑 version_drift.mjs → 报告 失效 的版本敏感知识
    → /kernel-learn 逐条：生成版本条件规则 + 多版本 eval case（旧版仍 pass + 新版也 pass）
    → 回归门：跑全版本树语料，任何版本退步则 revert，全过才 keep
    → 账本记"适配到 6.X"
  ```
- **信任衰减**（借 MemoryBank）：每条带 `last_validated_against: <version>`，目标树版本不同 → 降信任、强制重验后才引用。
- **度量加一维：跨版本通过率**（同题库在 6.1/6.6/6.12/7.0 各自通过率），证明真适配多版本而非只在一个版本对。
...
| `regression-gate.mjs` | **照骨架重写一份 kernel 版脚本(~144 行)** | 闸引擎写死 `visual-audit.mjs`(line 33)、测试对象发现写死 `skills/${skill}-design/references/canonical`(line 61)——都设计专属。控制流(记基线→重跑→退步即 fail)照抄，只换两处:闸 visual-audit → checkpatch+fact-gate；发现 网页 → eval-case |
...
| 3 | `[CLAIMS]` 答案契约写进 SKILL.md + `fact_gate.mjs` + `checkpatch_gate.sh` | 客观闸 | 1 |
| 4 | eval 语料种子（通用 kernel/BSP 任务 + 红线用例）+ objective/subjective 标签 | `tests/eval/` | 3 |
| 5 | 回归 runner + baseline（复用 regression-gate） | baseline | 4 |
| 6 | kernel critic 面板（4 轴） | critic | 4 |
| 7 | `/kernel-learn` 原子三件套 + 抽象去重 + 邻域 case | 命令 | 3,4,5 |
| 8 | ledger/rules：fires/catches + Laplace + 消融 prune（复用 evolve-ledger/rules） | 回归门 | 5,7 |
| 9 | （推后）多轮自纠 §6.5、metacognitive self_model、写侧 log 主指标曲线 | — | 8 |

步 1-2 是引擎能咬住东西的最小结构（现在单文件，回归门没有可把关的对象）。
...
| **P1 客观闸** ⭐ | 步3 + §6.10 树绑定 + 守#3 | fact-gate 对真树抓幻觉（地基规则，最大单点跃升） |
| **P2 语料+回归门** | 步4-5 + 守#1#2 | 通过率数字 + 坏改动自动 revert |
| **P3 critic+学习** | 步6-7 + 守#7#8#10 | 解坑 → 自动产笔记+fail→pass case |
| **P4 账本+版本适配** | 步8 + §6.9 + 守#4#5#6#9 | 跨版本通过率曲线 + drift 报告 |
| **P5 推后** | 多轮自纠/metacognitive/coccinelle/路由limits | — |

顺序：先 P0（回归门的前提，且单独有用）→ P1（fact-gate，演示价值最高）→ P2…。
...
- [ ] eval case 用通用占位 compatible/CONFIG，不绑真 SDK
...
- metacognitive `self_model.json` + `strategies.json`（论文 #7，effort L）：语料/规则规模大了再上。
- 编译闸要真 kernel 树 + 工具链，较重；先上 checkpatch + fact-gate，编译闸按需。
- ~~复用脚本是否需薄 fork~~ **已确认**（见 §8 复用边界表）：ledger 直接用、rules 小补丁、regression-gate 照骨架重写一份 kernel 版脚本。

**开发期测试树（不进 committed skill）：** `kernel-6.1`(Linux 6.1.141) + `u-boot`，自带 checkpatch.pl，开发时拿它当 `--tree` 跑 fact-gate / checkpatch / 编译闸。skill 保持树无关，该路径只在测试命令 / `.scratch/` 出现，对树只读。

---

## 12. GitHub Pages 展示计划

**时机：P1（fact-gate 能跑）之后才开始做**——页里放真跑的 fact-gate 抓幻觉，不做 画大饼（诚实状态，呼应"建好≠跑过"教训）。**3 页**，跟现有 `docs/HARNESS-ROADMAP.html`（design harness）平行——sky-skills 就有了 design + kernel 两个 harness，`index.html` 加链接。全部按团队标准：`anthropic-design` skill + 过三闸（verify / visual-audit / screenshot）+ 通用脱敏（无厂商名、无内部路径）。

| 页 | 内容 | 何时做 |
|---|---|---|
| **架构页** | 它是什么、完整循环图、为什么越用越聪明、引擎组件、与 design harness 同构 | P1 后 |
| **能力页** | 三道闸实跑、critic 面板、`/kernel-learn`、真实指标（通过率 / 账本） | P2-P3 后（有真数据再放，不放假图） |
...
- **fact-gate**：失败 grep 输出原样喂回下一轮（RISE）；auto-learn 产的邻域 case 入库前先过 gate，防伪造 case 污染语料（Test-Time SI）；自报"正确"在 grep 确认前都算未验证（MemGPT）；grep 靶子收窄到当前子系统 path-scope，更快且少误配（MoE-offload）；先跑确定性 gate，critic 只评 gate 判不了的残差（TurboQuant）。
- **critic-panel**：判别器在生成器外、绝不自评；每轴显式输入契约不重叠不留缝（MAS survey / AgentCoder）；阈值按本技能历史分布标定（低于 1σ 才 fail），而非固定线（Test-Time SI / Cherry-IFD）；周期拿 fact-gate 真值校 critic，校不准的轴降权（Metacognitive）；加一条"memory-applicability"轴——该用的笔记没被取用就 fail（Evo-Memory）。
- **eval-corpus**：objective/subjective 分标，keep/revert 只认 objective（RISE）；二元能力红线用例单独门（LLM-in-a-Flash blog：2-bit 看着没差却悄悄搞坏 tool calling）；Batch1(冷,fail) vs Batch2(笔记入库后,pass) 证明笔记真防复发（AgentFactory）；只收有区分度的 case（Cherry-IFD）；周期对最新树重验引用，防陈旧引用悄悄封顶（SPIN ceiling）；答得好的难任务回填成新冻结 case（Karpathy LLM-Wiki）。
- **账本+回归门**：这就是 AFlow/AgentFactory 的隔离实验循环 + Test-Time-SI 的 adapt-respond-RESET；git-revert 设为默认；per-note `s=(c_succ+1)/(c_use+2)`（EvolveR）；leave-one-out 消融找死笔记；fires(命中) vs catches(翻转 fail→pass) 区分，高 fires 零 catch = 死策略（Metacognitive/OMNE）；时间感强度 `R=exp(-(now-last_fired)/S)`，S 被 catches 和严重度（砖板/数据丢失类）放大（MemoryBank）；语料通过率随 commit = 主指标（Karpathy）。
- **auto-learn**：原子产出且拒绝无可执行检查的笔记（地基规则）；只在多轮把 turn-1 fail 转成 pass 时才落（RISE-Self）；建前用 base 技能跑一遍，确认真 fail 才建（Cherry-IFD）；蒸成抽象通用原则，具体只留 must-cite（EvolveR/OMNE）；建前语义去重（EvolveR）；生成 1-3 邻域 case 每条过 gate（Test-Time SI）；写侧 ingest：一个坑折进最相关笔记 + 受影响 SKILL 的 forbidden/handoff + 交叉引用（Karpathy）。
- **router**：单技能基本不适用；保留"按召回评分 + 不确定门"备未来若拆子域。
- **hindsight-memory**：笔记 = 散文 + 配对可执行谓词（`.check` 或 fact-gate 子句）；固定 front-matter；一行 grep 索引 + 完整体两层；每条带 specific 子句(gate 逐字 grep) + abstract 子句(子系统原则)；`index.md` 先读；prune 用 archive 不删，遇匹配的归档笔记复活而非重学。
- **other**：生成式 `self_model.json`（各路由/各轴/各版本真实统计，机器生成不手写）；显式 `strategies.json` 学习策略菜单，卡住换策略；外层学习循环改动本身也要过冻结语料才提交。

### A.2 判定低价值的论文（诚实记录）

TurboQuant（纯量化，全是工程纪律类比）、FlashMoE（训练的缓存预测器，过度设计）、autoresearch blog（链接博客，唯一有用的红线用例点是已知 eval 陷阱非论文贡献）、MoE-offload（GPU/SSD 推理管道，唯一有用的 recall 路由跟 LLM-in-a-Flash 重复）、MemGPT（运行时 context 分页，正交，且它那套 LLM 自判正是本引擎要不信任的）、SPIN（自博弈收敛理论在小 N 离散语料退化成"过冻结语料"，易过拟合）。
...
6. 刻意保留人 + 冻结语料在环里；按 Metacognitive 的定义保持"extrinsic"，不追求无人自治。
...
**采用 vs 自建结论：自建，不采用任何仓整体。** 四个全是 Karpathy autoresearch 那同一套保留/撤销循环（`evolve-ledger.mjs` 头部已注明出处）。每个都更弱或跨域：darwin-skill = 纯主观 LLM 评 SKILL 散文质量、零客观闸（自报准确率 46-74%）；autoresearch-skill / nuwa-skill = 纯文档无引擎代码，TSV 学到的丢弃、无冻结语料、无 fact-gate、无规则表。我们要建的（fact-gate / 编译 checkpatch 闸 / 冻结语料 / fires-catches 账本 / 原子三件套）正是它们都缺的。**§8 复用计划不变**（ledger 直用、rules 小补丁、regression 重写 kernel 版）。

**这些仓确认我们已有**（无需动作）：单改动一实验、冻结评审器、只追加账本 + 历史最好曲线、git-revert 默认、crash 第三态、fires/catches+Laplace+消融、盲多评委无自评、objective/subjective 分离且 keep/revert 锚客观半、原子三件套、结构闸、来源覆盖闸。

**冲突（保持我们设计，不要回退）**：darwin/autoresearch-skill 把 keep/revert 锚在 **100% 主观指标**上（darwin LLM 评散文；autoresearch-skill 信可作弊的用户 Verify 命令）——这正是我们 fact-gate 要防的。我们 keep/revert **必须锚在客观半**（fact-gate + checkpatch/compile），critic 只对主观残差当参考。

**净收获：12 个循环操作护栏**（确认设计文档和复用脚本里都没有）。优先序如下，折进对应组件：

| # | 护栏 | 落点 | 量 |
|---|---|---|---|
| 1 | **评审器自降解校准**：故意改坏 gold 答案 N 种，断言闸+critic 给每种打分严格低于 gold（证明"能分好坏"；区别于红线用例=好答案仍 pass，这是坏答案必 fail） | eval-corpus（§6.4，`tests/eval/degraded/`） | M |
| 2 | **dry-run 覆盖率天花板**：账本记每行是真跑客观闸还是只 critic；某轮 >30% 没跑闸 → 标"不可信、分数虚高" | ledger（§6.6，加 `eval_mode` 列） | S |
| 3 | **度量错误熔断**：分清"闸说 fail"和"闸自己崩了"（树缺/工具缺）；连崩 2 次 → 停回归门升级，不把这些写成 fail 污染账本 | fact-gate（§6.2，加 `gate-error` 终态） | S |
...
| 6 | **每轮超时预算**：每个 checkpatch/fact-gate/compile/critic 调用设超时；挂了就 kill→crash 丢这轮 | ledger（§6.3/§6.6） | M |
| 7 | **critic 噪音纪律**：主观轴的 kept 改进要超 min-delta 噪音底 + 确认重跑；主观退步要二次确认才 revert | critic（§6.5） | M |
| 8 | **scope/limits 字段**：每条知识带版本区间/arch 假设/"对 X 未验证"；结构闸要求非空（呼应 §6.9 版本标） | auto-learn（§6.7 模板） | S |
| 9 | **revert 历史回读**：提改动前先 grep 账本 revert 行,不重提死路 + surface 上次 kept diff | ledger（§6.6/§6.7） | S |
| 10 | **3 测准入过滤**：一个坑要变永久 case/rule 前测——通用性 / 预测性(对新答案也 fire) / 排他性(不被已有规则覆盖);部分过=降级软启发,全不过=丢 | auto-learn（§6.7，§9 步7） | M |
| 11 | **生成器自写领域检查器**：可机检的坑(atomic 里睡、漏 of_node_put、copy_to_user 没校验)→ 自动产 coccinelle/AST 小检查器当可执行物,不止 grep | auto-learn（§6.7 扩展，§9 步7） | L |
| 12 | **路由带 limits**：用户描述问题(非目标)时,need→子系统诊断表给路由 + 该路由的盲点(must-cite + "本路由不覆盖 X") | SKILL 路由层（§9 步1 顺手） | S |

前 6 个让无人值守的 `/kernel-learn` 回归门有界、可比、崩溃安全，先做；#10/#11/#12 扩展 地基规则，放到 auto-learn 建设（§9 步 7）。建法 §9 顺序不变。
